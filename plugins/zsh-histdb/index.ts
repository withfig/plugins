...
HISTDB_TABULATE_CMD=(sed -e $'s/\x1f/\t/g')
  ...
  configuration: [{
    ...
    description: "Pull zsh-autosuggestion suggestions from the histdb, using a pre-made or custom strategy.",
    type: "script",
    interface: "select",
    default: "none",
    options: [
      "none",
      {
        ...
        description: "Suggest the most frequently issued command in the current directory or any subdirectory."
      },
      {
        option: "frequency-in-directory",
        description: "Suggest the most frequently issued command issued exactly in this directory, or if no matches then the most frequent in any directory."
      },
      "custom",
    ],
    compile: (value) => {
      switch (value) {
        case "frequency-in-subdirectory":
          return getZshAutosuggestIntegrationForQuery(FREQUENCY_IN_SUBDIRECTORY_QUERY);
        case "frequency-in-directory":
          return getZshAutosuggestIntegrationForQuery(FREQUENCY_IN_CURRENT_DIRECTORY_QUERY);
        case "none":
        case "custom":
        default:
          return ""
    ...
    hidden: ({ config }) => config["zsh-autosuggestion-integration"] !== "custom",
    description: "Custom SQL query to get suggestions for zsh-autosuggestions.",
    type: "script",
    interface: "textarea",
    default: FREQUENCY_IN_SUBDIRECTORY_QUERY,
    compile: value => getZshAutosuggestIntegrationForQuery(value, "fig_custom_histdb_top"),
  }, {
    displayName: "History Reverse Search Keybindings",
    name: "history-reverse-search",
    description: "Enable history-reverse-isearch through the history db with ctrl-r",
    type: "script",
    interface: "toggle",
    default: false,
    compile: (value, { ctx }) => value
