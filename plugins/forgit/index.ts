const commandConfiguration = [
  ...
  const title = command.split("_").slice(1).map(
  ...
  const fzfOptionsItem: Fig.ConfigurationItem = {
    ...
    interface: "text",
    environmentVariable: `${command.toUpperCase()}_FZF_OPTS`,
    default: null,
  }
...
const plugin: Fig.Plugin = {
  ...
  installation: {
    origin: "github",
    bash: {
      postScript: ({ ctx }) => `export PATH="$PATH:${ctx.plugin.installDirectory}/bin`,
    ...
    fish: {
      postScript: ({ ctx }) => `set -Ua fish_user_paths "${ctx.plugin.installDirectory}/bin"`,
  ...
  configuration: [{
    ...
    type: "environmentVariable",
    description: "Adds default fzf options for `forgit`, including keybinds, layout, etc.  (No need to repeat the options already configured in `FZF_DEFAULT_OPTS`)",
    interface: "text",
    environmentVariable: "FORGIT_FZF_DEFAULT_OPTS",
    default: null,
    ...
    interface: "text",
    environmentVariable: "FORGIT_COPY_CMD",
    default: "xclip -selection clipboard",
  }, {
    displayName: "Pagers",
    description: "Forgit will use the default configured pager from git.  But this behavior can be configured with the following options",
    configuration: [{
      displayName: `Forgit Common Pager`,
      type: "environmentVariable",
      description: "Common pager. Falls back to `git config core.pager` or `cat`",
      interface: "text",
      environmentVariable: "FORGIT_PAGER",
      default: null,
    }, {
      ...
      interface: "text",
      environmentVariable: "FORGIT_SHOW_PAGER",
      default: null,
    }, {
      ...
      interface: "text",
      environmentVariable: "FORGIT_DIFF_PAGER",
      default: null,
    }, {
      displayName: "Pager for `gitignore`",
      type: "environmentVariable",
      description: "Falls back to `bat -l gitignore --color always` or `cat`",
      interface: "text",
      environmentVariable: "FORGIT_IGNORE_PAGER",
      default: null,
    }, {
      ...
      interface: "text",
      environmentVariable: "FORGIT_LOG_FORMAT",
