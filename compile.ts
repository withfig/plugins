...
const setDeveloperState = (key: string, value: string | null) => {
  try {
    return execSync(`fig _ local-state plugins.developer.${key} ${value === null ? "--delete" : value}`);
  } catch (_) {
    return null;
  }
}
...
async function runProgram() {
  ...
  if (isMacOS) {
    ...
    if (!fs.existsSync(globalFigAppPath) && !fs.existsSync(localFigAppPath)) {
      ...
      try {
        ...
        warn(
          ...
          chalk.bold(chalk.cyan("1. Run the install and update script ( ◧ > Integrations > Developer > Run install and update script)")),
    ...
    warn(
      ...
      chalk.bold(chalk.yellow("You can still build and contribute to completion specs, but you won't be able to test them unless you are on a mac or a macosVM"))
    );
  }
  ...
  if (isMacOS) {
    ...
    setDeveloperState(PLUGINS_DEV_STATE.PLUGINS_LAST_UPDATED, `${Date.now()}`);
    setDeveloperState(PLUGINS_DEV_STATE.PLUGIN_DIRECTORY, `${path.join(process.cwd(), DESTINATION_FOLDER_NAME, "plugins")}`);
